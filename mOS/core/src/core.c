...
#define	IS_HUGEPAGE 0
	...
	if (!(mtcp->bufseg_pool = MPCreate(sizeof(tcpbufseg_t),
			...
			((g_config.mos->rmem_size - 1) / sizeof(tcpbufseg_t) + 1), IS_HUGEPAGE))) {
		TRACE_ERROR("Failed to allocate ev_table pool\n");
		exit(0);
	}
	if (!(mtcp->sockent_pool = MPCreate(sizeof(struct sockent),
			sizeof(struct sockent) * g_config.mos->max_concurrency * 3, IS_HUGEPAGE))) {
		TRACE_ERROR("Failed to allocate ev_table pool\n");
		exit(0);
	}
	...
	if (g_config.mos->is_ips_mode) {
		for(i=0; i < g_config.mos->max_concurrency; i++) {
			mtcp->smap[i].arr_pkt_ctx = (struct pkt_ctx **)calloc(2048, sizeof(struct pkt_ctx *));
			mtcp->smap[i].pkt_idx = 0;
		}
	}
...
mtcp_getconf(struct mtcp_conf *conf)
{
	...
	conf->is_ips_mode = g_config.mos->is_ips_mode;
...
mtcp_setconf(const struct mtcp_conf *conf)
{
	...
	g_config.mos->is_ips_mode = conf->is_ips_mode;
