	...
	if (!TCP_SEQ_BETWEEN(pctx->p.seq + pctx->p.payloadlen, cur_stream->rcv_nxt, 
		...
		if (cur_stream->state == TCP_ST_ESTABLISHED) {
			...
			if (TCP_SEQ_LEQ(pctx->p.seq, cur_stream->rcv_nxt)) {
				cur_stream->actions |= MOS_ACT_SEND_ACK_AGG;
				//printf("MOS_ACT_SEND_ACK_AGG pctx->p.seq = %u cur_stream->rcv_nxt = %u\n", pctx->p.seq, cur_stream->rcv_nxt);
			} else {
				cur_stream->actions |= MOS_ACT_SEND_ACK_NOW;
				//printf("MOS_ACT_SEND_ACK_NOW\n");
			}
		} else {
				...
				struct pkt_ctx *pctx)
{
	...
	//printf("Start ProcessTCPPayload pctx->p.seq + pctx->p.payloadlen = %u, cur_stream->rcv_nxt = %u\n", pctx->p.seq + pctx->p.payloadlen, cur_stream->rcv_nxt);
	...
	if (TCP_SEQ_LT(pctx->p.seq + pctx->p.payloadlen, cur_stream->rcv_nxt)) {
		//printf("Drop and ACK NOW\n");
		return FALSE;
	}

///* 
	// Added
	if(g_config.mos->is_ips_mode) {
		if (TCP_SEQ_GT(pctx->p.seq, cur_stream->rcv_nxt)) {
			//printf("PACKET LOSS? Drop and ACK NOW:  pctx->p.seq = %u, cur_stream->rcv_nxt = %u\n", pctx->p.seq, cur_stream->rcv_nxt);
			return FALSE;
		}
	}
//*/
	...
	*/
	...
	if (TCP_SEQ_GT(pctx->p.seq + pctx->p.payloadlen, cur_stream->rcv_nxt + rcvvar->rcv_wnd) &&
				  (!g_config.mos->is_ips_mode || cur_stream->rcv_nxt != pctx->p.seq)) {
		/* MOS_ON_ERROR: payload outside the window arrives */
		printf("MOS_ON_ERROR???\n");
		#if 0
		if (cur_stream->side == MOS_SIDE_CLI) {
			printf("MOS_SIDE_CLI payload outside the window arrives: Never here\n");
			...
			printf("MOS_SIDE_SRV payload outside the window arrives: Never here\n");Frcv_nxt
			//printf("pctx->p.seq = %u, pctx->p.payloadlen = %u / cur_stream->rcv_nxt = %u, rcvvar->rcv_wnd = %u\n", 
			//			pctx->p.seq, pctx->p.payloadlen, cur_stream->rcv_nxt, rcvvar->rcv_wnd);
		...
		//printf("pctx->p.seq = %u, pctx->p.payloadlen = %u / cur_stream->rcv_nxt = %u, rcvvar->rcv_wnd = %u\n", 
		//				pctx->p.seq, pctx->p.payloadlen, cur_stream->rcv_nxt, rcvvar->rcv_wnd);
		printf("Exceeds receiving buffer Drop packet\n");
		pctx->forward = 0;
		return FALSE;
	}
	...
	if (off >= 0) {
		...
		if (HAS_STREAM_TYPE(cur_stream, MOS_SOCK_MONITOR_STREAM_ACTIVE)) {
			...
			if (fflen > 0) {
				...
				if (cur_stream->side == MOS_SIDE_CLI) {
					SOCKQ_FOREACH_REVERSE(walk, &cur_stream->msocks) {					
					if (walk->monitor_stream->peek_offset[cur_stream->side] < rb->head + fflen) {
							printf("\nWe detect buffer outrun: MOS_SIDE_CLI\n");
							//HandleCallback(mtcp, MOS_NULL, walk, cur_stream->side, pctx, MOS_ON_ERROR);
							printf("pctx->p.seq = %u, pctx->p.payloadlen = %u / cur_stream->rcv_nxt = %u, rcvvar->rcv_wnd = %u\n", 
										pctx->p.seq, pctx->p.payloadlen, cur_stream->rcv_nxt, rcvvar->rcv_wnd);
							printf("3. walk->monitor_stream->peek_offset[cur_stream->side] = %ld, rb->head = %ld, fflen = %d\n", 
									walk->monitor_stream->peek_offset[cur_stream->side], rb->head, fflen);
							pctx->forward = 0;
							printf("Make abort for debug\n");
							abort();
							return FALSE;
						}
					} SOCKQ_FOREACH_END;
					...
					SOCKQ_FOREACH_START(walk, &cur_stream->msocks) {
						if (walk->monitor_stream->peek_offset[cur_stream->side]	< rb->head + fflen) {
							#if 0
								printf("\nWe detect buffer outrun: MOS_SIDE_SRV\n");
								printf("pctx->p.seq = %u, pctx->p.payloadlen = %u / cur_stream->rcv_nxt = %u, rcvvar->rcv_wnd = %u\n", 
										pctx->p.seq, pctx->p.payloadlen, cur_stream->rcv_nxt, rcvvar->rcv_wnd);
								printf("3. walk->monitor_stream->peek_offset[cur_stream->side] = %ld, rb->head = %ld, fflen = %d\n", 
									walk->monitor_stream->peek_offset[cur_stream->side], rb->head, fflen);
	...
	if (cftail > 0 && (rcvvar->irs + 1) + cftail > cur_stream->rcv_nxt) {
		...
		//printf("[ProcessTCPPayload] Move rcv_nxt from %u to %ld.\n",
		//		cur_stream->rcv_nxt, (rcvvar->irs + 1) + cftail);
		cur_stream->rcv_nxt = (rcvvar->irs + 1) + cftail;
	}
	...
	if (TCP_SEQ_LEQ(cur_stream->rcv_nxt, prev_rcv_nxt)) {
		...
		//printf("\nThere are some lost packet: cur_stream->rcv_nxt = %u, prev_rcv_nxt = %u\n\n", cur_stream->rcv_nxt, prev_rcv_nxt);
		return FALSE;
	}
		...
		struct pkt_ctx *pctx)
{
	...
	//printf("Start Handle_TCP_ST_ESTABLISHED\n");
		...
		struct pkt_ctx *pctx)
{
	...
	if (cur_stream->state > TCP_ST_SYN_RCVD) {
		...
		if (!ret) {
			...
			//printf("[UpdateRecvTCPContext] Stream %d: Unexpected sequence: %u, expected: %u ignore the packet\n",
			//		cur_stream->id, pctx->p.seq, cur_stream->rcv_nxt);
#ifdef DBGMSG
			DumpIPPacket(mtcp, pctx->p.iph, pctx->p.ip_len);
#endif
