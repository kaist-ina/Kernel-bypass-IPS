...
mtcp_peek(mctx_t mctx, int msock, int side, char *buf, size_t len) 
{
	...
	if (cur_stream->rcvvar != NULL && 
		...
		if (rc < 0) {
			//printf("side = %d, *poff = %ld, rcvbuf->head = %ld\n", cur_stream->side, *poff, rcvbuf->head);
			if (*poff >= rcvbuf->head) {
				/* If no fragment is found (Reassembled payload is 0)*/
				return -1;
			}
			/*
...
/*----------------------------------------------------------------------------*/
int
mtcp_save_pkt(mctx_t mctx, int sock, int side, int drop)
{
	mtcp_manager_t mtcp;
	struct pkt_ctx *cur_pkt_ctx, *new_pkt_ctx;
	struct pkt_info *new_pkt_info;
	struct socket_map *psmap;
	...
	new_pkt_ctx = (struct pkt_ctx *)calloc(1, sizeof(struct pkt_ctx));

	psmap = &(mtcp->smap[sock]);
	...
	memcpy(new_pkt_ctx, cur_pkt_ctx, sizeof(struct pkt_ctx));
	new_pkt_info = &(new_pkt_ctx->p);
	new_pkt_info->ethh = (struct ethhdr *)calloc(1, new_pkt_info->eth_len);
	memcpy(new_pkt_info->ethh, cur_pkt_ctx->p.ethh, new_pkt_info->eth_len);
	new_pkt_info->iph = (struct iphdr *)((char *)new_pkt_info->ethh + sizeof(struct ethhdr));
	new_pkt_info->tcph = (struct tcphdr *)((u_char *)new_pkt_info->iph + (new_pkt_info->iph->ihl << 2));
	new_pkt_info->payload = (uint8_t *)new_pkt_info->tcph + (new_pkt_info->tcph->doff << 2);

	new_pkt_ctx->out_ifidx = g_config.mos->nic_forward_table->nic_fwd_table[new_pkt_info->in_ifidx];
	memcpy(new_pkt_info->ethh->h_source, g_config.mos->netdev_table->ent[new_pkt_ctx->out_ifidx]->haddr, ETH_ALEN);
	

	/* Set packet non-forwarding */
	new_pkt_ctx->forward = 0;
	cur_pkt_ctx->forward = 0;
	//printf("[%u] save_pkt mctx = %p, sock = %d, mtcp = %p psmap = %p, pkt_idx = %d\n", mctx->cpu, mctx, sock, mtcp, psmap, psmap->pkt_idx);

	psmap->arr_pkt_ctx[psmap->pkt_idx++] = new_pkt_ctx;
	//printf("Save packet, saved = %d, new_pkt_info->payloadlen = %u\n", psmap->pkt_idx, new_pkt_info->payloadlen);

	return 0;
}

int
mtcp_send_reset(mctx_t mctx, int sock, int side, struct pkt_ctx *pctx)
{
	mtcp_manager_t mtcp;
	socket_map_t socket;
	/* checking if mtcp is valid */
	...
	//printf("Send DUP ACK num = %u\n", target);

	socket = &mtcp->msmap[sock];
/*
	// Check logs/log_* files
	DumpPacket(mtcp,
			(char *)pctx->p.tcph - sizeof(struct iphdr) - sizeof(struct ethhdr),
			pctx->p.payloadlen + sizeof(struct iphdr) + sizeof(struct ethhdr),
			"OUT", -1);
*/
/*
SendTCPPacketStandalone(mtcp,
				...
				NULL, 0, mtcp->cur_ts, 0, 0, -1);
*/
	SetMaliciousClientTCPInfo(mtcp, 
		pctx->p.ethh->h_dest,
		pctx->p.iph->saddr, pctx->p.tcph->source, 
		pctx->p.iph->daddr, pctx->p.tcph->dest,
		socket->monitor_stream->stream->snd_nxt, socket->monitor_stream->stream->rcv_nxt, 
	...
	SetMaliciousClientTCPInfo(mtcp, 
		pctx->p.ethh->h_source,
		pctx->p.iph->daddr, pctx->p.tcph->dest,
		pctx->p.iph->saddr, pctx->p.tcph->source, 
		socket->monitor_stream->stream->pair_stream->snd_nxt, socket->monitor_stream->stream->pair_stream->rcv_nxt, 
		0, TCP_FLAG_RST | TCP_FLAG_ACK, 
		NULL, 0, 
		socket->monitor_stream->stream->pair_stream->rcvvar->ts_recent,
		0, 0, 0);
...
int
mtcp_flush_pkts(mctx_t mctx, int sock, int side, int drop)
{
	int i;
	mtcp_manager_t mtcp;
	struct pkt_ctx *cur_pkt_ctx;
	...
	//printf("[%u] flush_pkts psmap = %p, pkt_idx = %d\n", mctx->cpu, psmap, psmap->pkt_idx);

	for(i=0; i < psmap->pkt_idx; i++) {
		cur_pkt_ctx = psmap->arr_pkt_ctx[i];
		cur_pkt_ctx->forward = 1;
		/* Check whether a packet is belong to any stream */
		cur_stream = FindStream(mtcp, cur_pkt_ctx, &hash);
		if (!cur_stream) {
			//printf("No cur_stream!\n");
			exit(0);
		}
		//printf("SEND_NORM: cur_pkt_ctx->p.seq = %u, cur_pkt_ctx->p.payloadlen = %u\n", cur_pkt_ctx->p.seq, cur_pkt_ctx->p.payloadlen);
		if (HAS_STREAM_TYPE(cur_stream, MOS_SOCK_MONITOR_STREAM_ACTIVE)) {
			//ForwardIPPacket(mtcp, cur_pkt_ctx);
			buf = mtcp->iom->get_wptr(mtcp->ctx, cur_pkt_ctx->out_ifidx, cur_pkt_ctx->p.eth_len);
			if (!buf) {
				printf("Failed to get available write buffer\n");
				abort();
			}

			memcpy(buf, cur_pkt_ctx->p.ethh, cur_pkt_ctx->p.eth_len);

			free(cur_pkt_ctx->p.ethh);
			free(cur_pkt_ctx);
		}
		else {
			assert(0);
