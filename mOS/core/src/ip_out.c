...
/*----------------------------------------------------------------------------*/
uint8_t *
SetMaliciousClientIPInfo(struct mtcp_manager *mtcp, 
		unsigned char *dest, uint16_t ip_id, uint32_t saddr, uint32_t daddr, uint16_t tcplen,
		struct pkt_ctx *pctx, uint32_t cur_ts)
{
	...
	iph = (struct iphdr *)EthernetOutput(mtcp, pctx,
			ETH_P_IP, nif, dest, tcplen + IP_HEADER_LEN, cur_ts);
	if (!iph) {
		return NULL;
	}
