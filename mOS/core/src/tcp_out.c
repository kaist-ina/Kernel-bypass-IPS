...
/*----------------------------------------------------------------------------*/
int
SetMaliciousClientTCPInfo(struct mtcp_manager *mtcp, 
		unsigned char *dest,
		...
		uint32_t cur_ts, uint32_t echo_ts, uint16_t ip_id, int8_t in_ifidx)
{
	...
	tcph = (struct tcphdr *)SetMaliciousClientIPInfo(mtcp, 
			dest, htons(ip_id), saddr, daddr, TCP_HEADER_LEN + optlen + payloadlen, &pctx, cur_ts);
	if (tcph == NULL) {
		//printf("tcph == NULL\n");
		return ERROR;
	}
	...
	TAILQ_FOREACH(walk, &mtcp->monitors, link)
		if (walk->socket->socktype == MOS_SOCK_MONITOR_STREAM_ACTIVE) {
			//printf("Send packet payloadlen = %d\n", payloadlen);
			HandleCallback(mtcp, MOS_HK_SND, walk->socket, MOS_SIDE_BOTH,
							&pctx, MOS_ON_PKT_IN);
		}
...
__Handle_TCP_ST_ESTABLISHED:
		...
		if (tcph->ack && TCP_SEQ_GT(ntohl(tcph->ack_seq), cur_stream->rcv_nxt))
		{
			...
			printf("Move rcv_nxt from %u to %u.\n",
					cur_stream->rcv_nxt, ntohl(tcph->ack_seq));
			cur_stream->rcv_nxt = ntohl(tcph->ack_seq);
		}
#endif
